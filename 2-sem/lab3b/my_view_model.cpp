...
void MyViewModel::edit_items_in_view(const Message& message, int row)
{
    add_item_to_view(message.get_sender_info().get_string(), row, item_columns["Sender"]);
    add_item_to_view(size_to_qstring(message.get_message_size()), row, item_columns["Size"]);
    add_item_to_view(message_type_to_qstring(message.get_message_type()), row, item_columns["Message type"]);
    QDateTime date_time = QDateTime::fromString(message.get_creating_time(), get_date_time_format());
